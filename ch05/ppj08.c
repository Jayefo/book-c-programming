...
int dTime1 = 8 * 60, dTime2 = 9 * 60 + 43, dTime3 = 11 * 60 +19, dTime4 = 12 * 60 + 47, dTime5 = 14 * 60,
  dTime6 = 15 * 60 + 45, dTime7 = 19 * 60, dTime8 = 21 * 60 + 45, hh, mm, cvMM;
    ...
    printf("Enter a 24-hour time (hh:mm): ");        // input curretn time 24 hours format
    scanf("%2d:%2d", &hh, &mm);
    ...
    if (abs(cvMM - dTime1) <= abs(cvMM - dTime2))       // abs() funtion for minus result to plus
        printf("Closest a 24-hour time is 8:00 a.m., arriving at 10:16 a.m.\n");
    else if (abs(cvMM - dTime2) <= abs(cvMM - dTime3))  
        printf("Closest a 24-hour time is 9:43 a.m., arriving at 11:52 a.m.\n");
    else if (abs(cvMM - dTime3) <= abs(cvMM - dTime4))  
        printf("Closest a 24-hour time is 11:19 a.m., arriving at 1:31 p.m.\n");
    else if (abs(cvMM - dTime4) <= abs(cvMM - dTime5))  
        printf("Closest a 24-hour time is 12:47 p.m., arriving at 3:00 p.m.\n");
    else if (abs(cvMM - dTime5) <= abs(cvMM - dTime6))  
        printf("Closest a 24-hour time is 2:00 p.m., arriving at 4:08 p.m.\n");
    else if (abs(cvMM - dTime6) <= abs(cvMM - dTime7))  
        printf("Closest a 24-hour time is 3:45 p.m., arriving at 5:55 p.m.\n");
    else if (abs(cvMM - dTime7) <= abs(cvMM - dTime8)) 
        printf("Closest a 24-hour time is 7:00 p.m., arriving at 9:20 p.m.\n");
    else
        printf("Closest a 24-hour time is 9:45 p.m., arriving at 11:58 p.m.\n");
